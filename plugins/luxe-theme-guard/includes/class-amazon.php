 ...
 */
class Luxe_Theme_Guard_Amazon {

	const TAG            = 'luxetrendse0f-20';
	const MARKET         = 'www.amazon.com';
	const OPTION_LAST    = 'luxe_theme_guard_amazon_last';
	const OPTION_LOG     = 'luxe_theme_guard_amazon_log';
	const OPTION_CURSOR  = 'luxe_theme_guard_amazon_cursor';
	const LOCK           = 'luxe_theme_guard_amazon_lock';
	...
	public static function build( $state ) {
		...
		$signals   = array(
			...
			self::row( 'tag', 'Partner tag lock', $tag_ok, $tag_ok ? ( 'tag=' . $tag ) : ( 'Last Amazon URL is missing tag=' . $tag . '. Guard did not rewrite it.' ) ),
			...
			self::row( 'cycle', 'One Amazon item per cycle', $armed, 'Same 5-minute WP-Cron as Theme Guard. Never on a shopper page view.' ),
		);
		$green = 0;
	...
	/**
	 * Register the admin cycle button.
	 */
	...
	public function maybe_manual() {
		if ( empty( $_POST['luxe_theme_guard_amazon_one'] ) ) {
			return;
		}
		...
		add_settings_error(
			...
			sprintf(
				...
				__( 'Amazon AI cycle finished. ASIN %1$s. %2$d / %3$d Amazon signals green. URLs not rewritten. Status preserved.', 'luxe-theme-guard' ),
				$asin ? $asin : 'none',
				$green,
				$total
			),
	...
	/**
	 * One catalog item. Nested calls skip the lock (theme 24/7 already holds one).
	 *
	...
	public function cycle_one( $source = 'cron', $nested = false ) {
		if ( class_exists( 'Luxe_Theme_Guard_Plugin' ) && ! Luxe_Theme_Guard_Plugin::instance()->enabled( 'amazon_ai' ) ) {
			$board = self::build(
				array(
					'armed'     => false,
					'tag'       => self::wanted_tag(),
		...
		if ( $asin && class_exists( 'Luxe_Theme_Guard_Amazon_API' ) && Luxe_Theme_Guard_Amazon_API::has_credentials() ) {
			$hit = Luxe_Theme_Guard_Amazon_API::get_item( $asin, $tag );
