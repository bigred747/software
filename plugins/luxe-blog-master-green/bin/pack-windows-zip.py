...
SLUG = "luxe-blog-master-green"
DEST_DIR = ROOT.parents[1]
OUTPUTS = [
    DEST_DIR / "luxe-blog-master-green.zip",
    DEST_DIR / "Luxe-Blog-Master-Green.zip",
    DEST_DIR / "dist" / "Luxe-Blog-Master-Green-v2.9.0.zip",
    DEST_DIR / "luxe-blog-master-green-windows11-wordpress.zip",
]
TARGET = 145 * 1024
...
def png_pad(png: bytes, extra: int) -> bytes:
    ...
    extra = max(0, extra)
    payload = b"Comment\x00Luxe Blog Master Green 2.9.0 Windows 11 WordPress package." + (b"\n" * extra)
...
def main() -> None:
    ...
    for name, data, _store in with_icon(files, packed):
        if name == f"{SLUG}/{SLUG}.php":
            header = data
            break
    if not header or b"Plugin Name: Luxe Blog Master Green Board" not in header:
