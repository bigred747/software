#!/usr/bin/env python3
"""Build a WordPress PclZip plugin zip (PKZIP 2.0, MS-DOS)."""
...
SLUG = "luxe-download-scout"
DEST_DIR = ROOT.parents[1]
OUTPUTS = [
    DEST_DIR / "luxe-download-scout.zip",
    DEST_DIR / "dist" / "Luxe-Download-Scout-v1.1.1.zip",
]
...
def main() -> None:
    ...
    for name, data, _store in files:
        if name == f"{SLUG}/{SLUG}.php":
            header = data
            break
    if not header or b"Plugin Name: Luxe Download Scout" not in header:
