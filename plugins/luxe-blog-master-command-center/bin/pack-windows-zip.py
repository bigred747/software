...
OUTPUTS = [
    ...
    DEST_DIR / "Luxe-Blog-Master-Command-Center.zip",
    DEST_DIR / "dist" / "Luxe-Blog-Master-Command-Center-v2.9.0.zip",
...
def png_pad(png: bytes, extra: int) -> bytes:
    ...
    extra = max(0, extra)
    payload = b"Comment\x00Luxe Blog Master Command Center 2.9.0 Windows 11 WordPress package." + (b"\n" * extra)
...
def main() -> None:
    ...
    if b"Version: 2.9.0" not in header:
