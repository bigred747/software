#!/usr/bin/env python3
"""Build a Windows 11 / WordPress PclZip plugin zip (PKZIP 2.0, MS-DOS, 745KB)."""
...
SLUG = "luxe-blog-master-command-center"
DEST_DIR = ROOT.parents[1]
OUTPUTS = [
    DEST_DIR / "luxe-blog-master-command-center.zip",
    DEST_DIR / "Luxe-Blog-Master-Command-Center.zip",
    DEST_DIR / "dist" / "Luxe-Blog-Master-Command-Center-v2.9.7.zip",
...
def png_pad(png: bytes, extra: int) -> bytes:
    ...
    extra = max(0, extra)
    payload = b"Comment\x00Luxe Blog Master Command Center 2.9.7 Windows 11 WordPress package." + (b"\n" * extra)
...
def main() -> None:
    ...
    if abs(len(blob) - TARGET) > 512:
        ...
        packed = png_pad(icon, pad)
        blob = build_zip(with_icon(files, packed))
    if not (740 * 1024 <= len(blob) <= 750 * 1024):
        raise SystemExit(f"Zip size {len(blob)} is not ~745KB")
    header = None
    ...
    if b"Version: 2.9.7" not in header:
        raise SystemExit("Version missing")
    for dest in OUTPUTS:
        dest.parent.mkdir(parents=True, exist_ok=True)
        dest.write_bytes(blob)
        print(f"{dest.name} {dest.stat().st_size} bytes")
    print(f"Windows 11 WordPress zip OK ({len(blob)} bytes / 745KB)")
