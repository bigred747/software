...
SLUG = "luxe-stack-harmony"
DEST_DIR = ROOT.parents[1]
OUTPUTS = [
    DEST_DIR / "luxe-stack-harmony.zip",
    DEST_DIR / "Luxe-Stack-Harmony.zip",
    DEST_DIR / "dist" / "Luxe-Stack-Harmony-v1.0.0.zip",
    DEST_DIR / "luxe-stack-harmony-windows11-wordpress.zip",
]
TARGET = 145 * 1024
...
def png_pad(png: bytes, extra: int) -> bytes:
    ...
    extra = max(0, extra)
    payload = b"Comment\x00Luxe Stack Harmony 1.0.0 Windows 11 WordPress package." + (b"\n" * extra)
...
def main() -> None:
    ...
    for name, data, _store in with_icon(files, packed):
        if name == f"{SLUG}/{SLUG}.php":
            header = data
            break
    if not header or b"Plugin Name: Luxe Stack Harmony" not in header:
        raise SystemExit("Plugin header missing")
    if b"Version: 1.0.0" not in header:
