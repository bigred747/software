 ...
 */
class Luxe_Score_Repair_Amazon_API {

	const TOKEN_URL = 'https://api.amazon.com/auth/o2/token';
	const GET_ITEMS = 'https://creatorsapi.amazon/catalog/v1/getItems';
	const OPTION_TOKEN = 'luxe_score_repair_amazon_token';

	/**
	 * @return bool
	 */
	public static function has_credentials() {
		$settings = class_exists( 'Luxe_Score_Repair_Plugin' ) ? Luxe_Score_Repair_Plugin::instance()->settings() : array();
	...
	public static function get_item( $asin, $tag ) {
		$asin = Luxe_Score_Repair_Amazon::extract_asin( $asin );
		if ( ! $asin || ! self::has_credentials() ) {
			return array();
		}
		if ( ! Luxe_Score_Repair_Amazon::may_request( self::GET_ITEMS ) || ! Luxe_Score_Repair_Amazon::may_request( self::TOKEN_URL ) ) {
			return array();
		}
		$token = self::token();
		if ( '' === $token ) {
		...
		$response = wp_remote_post(
			...
			array(
				...
				'headers' => array(
					...
					'x-marketplace' => Luxe_Score_Repair_Amazon::MARKET,
				),
				'body'    => wp_json_encode(
					array(
						'itemIds'     => array( $asin ),
						'itemIdType'  => 'ASIN',
						'marketplace' => Luxe_Score_Repair_Amazon::MARKET,
						'partnerTag'  => Luxe_Score_Repair_Amazon::sanitize_tag( $tag ),
						'resources'   => array(
							'itemInfo.title',
